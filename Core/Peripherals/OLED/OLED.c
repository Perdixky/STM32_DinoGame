...
void OLED_SendCmd(uint8_t cmd)
{
	HAL_I2C_Mem_Write(OLED_HANDLE, OLED_ADDRESS, 0x00, 1, &cmd, 1, 10);
}
 ...
 * 2. 使用波特律动LED取模工具生成字模(https://led.baud-dance.com)
 *
 * @note 只需多次调用此函数即可实现滚动，不调用则暂停
 * @note 搭配使用HAL_Delay函数即可实现控制滚动速度
 */
void OLED_PrintRollingASCIIString(uint8_t y, char* str, const ASCIIFont* font, OLED_ColorMode color)
{
	//刷新相应的显存区域
	OLED_DrawFilledRectangle(0, y, 128, font->h, OLED_COLOR_REVERSED);
	//字符串x坐标
	static short X_Coordinate = 128;

	//显示完后被丢弃的字符数量
	static unsigned char Abandoned_charnum = 0;
	//用static关键字实现每次调用函数时自动移动位置
	X_Coordinate --;

	//显示完的字符丢弃
	if(X_Coordinate <= -font->w)
	{
		X_Coordinate = 0;
		Abandoned_charnum ++;
	}
	//屏幕最大显示字节数量，加1是因为由显示时会出现两端有半个字节的情况
	const unsigned char MAX_ShowNum = 128 / font->w + 1;
	//使用柔性数组
	static char* Cut_Substring;
	Cut_Substring = (char* )malloc(MAX_ShowNum);
	//截取适当的长度
	strncpy(Cut_Substring, str + Abandoned_charnum,  MAX_ShowNum);
	memset(Cut_Substring + MAX_ShowNum, '\0', 1);
	OLED_PrintASCIIString(X_Coordinate, y, Cut_Substring, font, color);
	if(Abandoned_charnum == strlen(str))//当字符串全部显示完毕，重置
	{
		Abandoned_charnum = 0;
		X_Coordinate = 129 - font->w;
	}
	free(Cut_Substring);
	//手动释放内存
}
